...
data["SVM"].append(float(svm_report[-35:-30]))
...
data["MultinomialNB"].append(float(mul_report[-35:-30]))
...
data["Tree"].append(float(tree_report[-35:-30]))
...
data["logistic"].append(float(logistic_report[-35:-30]))
...
plt.bar(X1, data["SVM"], color = 'silver', width = .3, label ='SVM')
plt.bar(X2, data["MultinomialNB"], color = 'gold',  width = .3, label =u'贝叶斯')
plt.bar(X3, data["Tree"], color = 'aquamarine', width = .3, label = u'决策树')
plt.bar(X4, data["logistic"], color = 'hotpink', width = .3, label ='logistic')
plt.title(u'算法结果对比图')
